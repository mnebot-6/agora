...
### Tarea 6: Aplicar la migración de flujos y comprobarla

**Ficheros:**
- Crear: `docs/payment_modes_check.sql`
...
Esperado: se aplica sin error. Si `CREATE OR REPLACE` se queja de que no puede cambiar el
tipo de retorno, has cambiado una firma sin querer: revísala contra el original.

- [ ] **Paso 2: Escribir el script de comprobación**

Crear `docs/payment_modes_check.sql`. Copia el estilo exacto de
`docs/stripe_release_offers_check.sql`: `ON_ERROR_STOP off` (para que un caso que falla no
aborte el resto), `\echo` como cabecera de cada caso, y **cada llamada autenticada en su
propio `BEGIN; SET LOCAL role=authenticated; SET LOCAL request.jwt.claims = ...; COMMIT;`**,
porque `SET LOCAL` solo vive dentro de una transacción.

Ojo con dos detalles que se equivocan solos: el rol de miembro corriente en
`community_members` es `'user'`, no `'member'`; y el `INSERT` en `auth.users` necesita la
lista larga de columnas, no solo `id` y `email`.
...
```bash
psql "$SUPABASE_DB_URL" -f docs/payment_modes_check.sql
```

Esperado: cada `SELECT` devuelve el valor anotado en su comentario. Si el caso 6 devuelve
`available` en vez de `reserved`, `reserve_slot` no está tomando la rama del relevo:
revisa `v_is_takeover`.

- [ ] **Paso 4: Commit**

```bash
git add docs/payment_modes_check.sql
git commit -m "test(pagos): script de comprobacion de los tres modos"
```
